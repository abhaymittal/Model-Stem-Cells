...
int StemCell::divide(StemCell& daughterCell, AutomatonCell ***environment, SimulationParameters sim) {
    int temp;
    Point childPoint;
...
int StemCell::divide(TACell& daughterCell, AutomatonCell*** environment,SimulationParameters sim) {
    int temp;
    Point childPoint;
    temp= getFavorableLocation(childPoint,environment, sim);
    if(temp!=0)
